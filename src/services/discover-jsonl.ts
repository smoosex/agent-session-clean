...
export async function discoverJsonlFiles(rootPath: string, agentLabel: string, signal?: AbortSignal): Promise<DiscoveredJsonlFiles> {
  ...
  try {
    ...
    issues.push({ path: root, message: code === "ENOENT" ? `${agentLabel} session directory does not exist` : `Cannot access ${agentLabel} session directory: ${error instanceof Error ? error.message : String(error)}`, severity: "error" })
    return { files, issues }
  }
  if (!rootStat.isDirectory()) {
    issues.push({ path: root, message: `${agentLabel} session path is not a directory`, severity: "error" })
    return { files, issues }
  }
